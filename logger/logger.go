...
type AvisLogger interface {
	Logger
	WithCategory(cat string) Logger
}

// NewAvisLogger avis用のlogger作成
func NewAvisLogger() AvisLogger {
	return &avisLogger{
		Logger: NewLogger(),
	}
}

type avisLogger struct {
	Logger
}

func (l *avisLogger) WithCategory(category string) Logger {
	return l.AddOption(map[string]interface{}{
		AVISCAT: category,
	})
}
